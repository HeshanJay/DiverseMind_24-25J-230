...
const MemoryTestActivity = ({ onNext, onBack }) => {
  ...
  useEffect(() => {
    let interval;
    if (showImage) {
      setTimer(2); // Set initial image display time
      interval = setInterval(() => {
        setTimer((prevTimer) => {
          if (prevTimer === 1) {
            setShowImage(false);
            setShowAnswers(true);
            setTimer(2); // Reset timer for answers
          }
          return prevTimer - 1;
        });
      }, 1000);
      ...
      interval = setInterval(() => {
        setTimer((prevTimer) => {
          if (prevTimer === 1) {
            handleAnswerClick(null); // Auto-submit if time runs out
          }
          return prevTimer - 1;
        });
      }, 1000);
  ...
  const handleAnswerClick = (answerId) => {
    // Clear timer and process answer
    setTimer(0);
    if (answerId === questions[currentQuestion].correctAnswer) {
      setMemoryScore((prev) => prev + 0.2);
    }
