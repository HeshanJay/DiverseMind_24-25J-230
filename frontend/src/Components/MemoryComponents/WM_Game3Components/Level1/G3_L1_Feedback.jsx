...
const G3_L1_Feedback = ({ totalScore = 0, handleRetry }) => {
  const navigate = useNavigate();
  const maxScore = 12; // 3 questions × 4 marks each

  // Star rating logic
  const filledStars = (() => {
    if (totalScore === 12) return 5;
  ...
  const feedbackMessage = () => {
    if (totalScore === 12) return "විශිෂ්ටයි! ඔබ සාර්ථකයි";
    if (totalScore >= 9) return "ඉතා හොඳයි! ඔබ එය හොඳින් කළා";
    if (totalScore >= 6) return "හොඳයි! ඔබ එය හොඳින් කළා";
...
G3_L1_Feedback.propTypes = {
  totalScore: PropTypes.number,
  handleRetry: PropTypes.func,
};

export default G3_L1_Feedback;