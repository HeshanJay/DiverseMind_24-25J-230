...
const G2_L3_Feedback = ({ totalScore = 0, handleRetry }) => {
  const navigate = useNavigate();
  const maxScore = 20; // 4 questions × 5 marks each
  ...
  const feedbackMessage = () => {
    if (totalScore === 20) return "විශිෂ්ටයි! ඔබ සාර්ථකයි";
    if (totalScore >= 15) return "ඉතා හොඳයි! ඔබ එය හොඳින් කළා";
...
G2_L3_Feedback.propTypes = {
  totalScore: PropTypes.number,
  handleRetry: PropTypes.func,
};

export default G2_L3_Feedback;