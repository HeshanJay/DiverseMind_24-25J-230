...
//                                 <button
//                                   onClick={() => handleAnswerClick(index)}
//                                   className="bg-gradient-to-r from-green-400 to-blue-500 text-white px-12 py-8 rounded-xl text-xl shadow-lg hover:scale-110 transition-transform flex items-center justify-start"
...
//                                     style={{ width: "320px", height: "70px" }}
//                                   >
//                                     <strong className="ml-4">{answers[currentQuestion - 1][index + 1]}</strong>
//                                   </button>
//                                 </td>
//                               )}
//                             </tr>
...
const AudioMeasurementActivity = ({ onNext, onBack }) => {
  ...
  const handleAnswerClick = (answerIndex) => {
    if (answerIndex === correctAnswers[currentQuestion - 1]) {
      // For audio test, add 0.2 points per correct answer.
      setAudioDiscriminationScore((prev) => prev + 0.2);
      setScore((prev) => prev + 0.2);
    }
    handleNextQuestion();
